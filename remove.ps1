...
if (!(Confirm -Question "This will delete .\docker\**\*, .\Dockerfile and msbuild files.`n`nAre you really completely sure you want to do that?" -DefaultYes)) {
...
if ((Test-Path .\*.sln) -and (Confirm -Question "Would you also like to delete the solution files and SRC folder?" -DefaultYes)) {
    ...
    if (Test-Path 'jss-app') {
        Remove-Item 'jss-app' -Force -Recurse
    }

    if (Test-Path ".\nuget.config") {
        Remove-Item 'nuget.config' -Force -Recurse
    }
